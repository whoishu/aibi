...
class OpenSearchService:
            ...
            index_body = {
                ...
                "mappings": {
                    "properties": {
                        ...
                        "vector": {"type": "knn_vector", "dimension": self.vector_dimension},
    ...
    def index_document(
        ...
        try:
            ...
            self.client.index(index=self.index_name, id=doc_id, body=doc, refresh=True)
    ...
    def keyword_search(
        self, query: str, size: int = 10, min_score: float = 0.1
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            search_body = {
                "query": {
                    "bool": {
                        "should": [
                            {"match_phrase_prefix": {"text": {"query": query, "boost": 3.0}}},
                            {
                                "match": {
                                    "text": {"query": query, "boost": 2.0, "fuzziness": "AUTO"}
                                }
                            },
                            {"term": {"keywords": {"value": query, "boost": 4.0}}},
                        ],
    ...
    def vector_search(
        self, query_vector: List[float], size: int = 10, min_score: float = 0.1
    ) -> List[Dict[str, Any]]:
        ...
        try:
            search_body = {
                "query": {"knn": {"vector": {"vector": query_vector, "k": size}}},
                "size": size,
                "min_score": min_score,
            }
    ...
    def hybrid_search(
        ...
        for doc_id, data in combined_scores.items():
            combined_score = (
                data["keyword_score"] * keyword_weight + data["vector_score"] * vector_weight
            )
