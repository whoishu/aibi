    ...
    try:
        ...
        if config.autocomplete.enable_personalization:
            personalization_service = PersonalizationService(
                redis_host=config.redis.host, redis_port=config.redis.port, redis_db=config.redis.db
            )
...
if __name__ == "__main__":
    ...
    uvicorn.run("app.main:app", host=config.api.host, port=config.api.port, reload=True)
